...
//         \/        \/       \/        \/        \/        \/        \/ 
//
// By Christophe Kohler (christophe.kohler@gmail.com) / Resistance
...
// -- Important defines
#define ISSIDEB // Use this to build disk B
// To allow skip level type in game "SOS"
// --------------------
...
//#define DIFFICULTY1 // HARD for QTE RANDOM AT END
#define DIFFICULTY2 // MEDIUM for QTE RANDOM AT END
//#define DIFFICULTY3 // EASY for QTE RANDOM AT END

// -- Debug
//#define ALLOWSKIPLEVEL // Cheat code. We can skip level in all cases with key "N"
//#define NOINTRO
//#define STARTLEVELH '9'
//#define STARTLEVELL '1'
// ---------------------
...
extern void loadtimeupdeaths(void); // Load in 0x9400
...
__at(0x9400) u8* gTimeUpDeathsPtr[7]; // 14 bytes
__at(0x940E) u8  gTimeUpMessages[512*3-(7*2)]; // The data.
...
// -------------------------------------------------------
// From cursor position get pointed zone
...
// -------------------------------------------------------
// This is main interruption (triggered 6 times per image).
// We use it to change game mode (0 on top, and 1 on bottom)
// To play music and to change palettes
// Each time interruption is triggered we change a counter.
// So we know position on screen with that counter.
// -------------------------------------------------------
...
// -------------------------------------------------------
// Set inteerupt handle, wait VSYNC to do it
...
// -------------------------------------------------------
// Dislplay art for banner. Central part is fills, only borders are displayed with arts
void DisplayBanner()
{
		// -- Display Banner
		u8* pvideomem;    
...
// -------------------------------------------------------
// Compute and display the stats about the number of deaths of player
...
// -------------------------------------------------------
// Restore back picture block under last displayed icon
...
// -------------------------------------------------------
// Set offset of image on screen.
// All functions are the same for the 3 possible image positions.
...
// -------------------------------------------------------
// Initialise the next action.
// Read a line in the "scenario" and do inits and set states.
...
#ifdef DIFFICULTY1	 // Hard		
			gQte_next = 100 + ( cpct_getRandom_lcg_u8(0) >> 3 ); // 100 + 64 = 2 to 3s
#endif	
#ifdef DIFFICULTY2 // Medium			
			gQte_next = 142 + ( cpct_getRandom_lcg_u8(0) >> 3 ); // 150 + 64 = 3 to 5s
#endif	
#ifdef DIFFICULTY3 // Easy			
			gQte_next = 185 + ( cpct_getRandom_lcg_u8(0) >> 3 ); // 185 + 64 = 4s to 6s
#endif			
...
// -------------------------------------------------------
// Erase all the bottom of screen
void EraseBanner()
{
	u8* pvideomem;    // Pointer to video memory
	pvideomem = cpct_getScreenPtr(CPCT_VMEM_START, 0,BANNERSTARY ); // Corner top left
...
// -------------------------------------------------------	
// Play death digitized voice, with random parameters		
...
// MAIN LOOP
//
// All the game code is here. This is a global unique loop
// that manage all levels (and intro of side B)
// -------------------------------------------------------
void main(void) 
{
	// Set variables and init them
	...
	gSfxDeathCount=0; // USed to play digitized voice sometimes (each 5 deaths)
	
	gNbDeath=0; //count how much time player is dead.
	...
	cpct_setVideoMode(0); // Mode 0 16 colors
	cpct_setBorder(HW_BLACK); // Black	

#ifdef ISSIDEB	// Side A do not have intro
#ifndef NOINTRO	
	// -- Load Title Screen, only for side B
	cpct_setPalette(palettetitle, 16); 
	cpct_clearScreen_f64(0); // First color of palette	
	asm_FdcOn();
	cpct_setBorder(HW_BLACK); // Black before load, will be RED after	
	...
	// Load TimeUp texts
	asm_FdcOn(); // Start motor (not stop because we will load another file just after)
	cpct_setBorder(HW_BLACK); // Black before load, will be RED after if error
	loadtimeupdeaths();
	// Init ptr
	for (x=0; x<7; x++)
		gTimeUpDeathsPtr[x] += 0x940E; // Add base adress to offset, and 14 bytes for pointer zone (first offset is 0)
	...
	while(1)  // General loop, including level load and change.
	{
		...
		while(shouldloopingame) // This is false when we change level
		{
			...
			else if (cpct_isAnyKeyPressed_f())
			{
				// is user activating the cheat code ?
				// Cheat code activation is "SOS"
				// gCheatSequenceCorrectLetters
				if ( gCheatSequenceCorrectLetters == 0 && (cpct_isKeyPressed(Key_S) ) )
					gCheatSequenceCorrectLetters = 1;
				else if ( gCheatSequenceCorrectLetters == 1 && (cpct_isKeyPressed(Key_S) ) )
				{
					// do nothing, wait that S stop beeing pressed
				}				
				else if ( gCheatSequenceCorrectLetters == 1 && (cpct_isKeyPressed(Key_O) ) )
					gCheatSequenceCorrectLetters = 2;
				else if ( gCheatSequenceCorrectLetters == 2 && (cpct_isKeyPressed(Key_O) ) )
				{
					// do nothing, wait that S stop beeing pressed
				}				
				else if ( gCheatSequenceCorrectLetters == 2 && (cpct_isKeyPressed(Key_S) ) )
				{
