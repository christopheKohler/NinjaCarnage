...
//#define LANGAGE 22 // Maltese

#define NUMBERLANGUAGE 7

int gLanguageColumn[NUMBERLANGUAGE] = { 3,4,7,10,13,16,19 };
...
string gStringIntro[NUMBERLANGUAGE];
vector<string> gStringTexts[NUMBERLANGUAGE];
vector<string> gStringDeaths[NUMBERLANGUAGE];
vector<string> gStringTimeUpDeaths[NUMBERLANGUAGE];
...
struct levelbinarymultilangage
{
	unsigned short pointers[NUMBERLANGUAGE][NBPOINTER]; // 140*NUMBERLANGUAGE bytes
	unsigned char data[10240*NUMBERLANGUAGE]; // Default buffer size (MAX)
};
...
levelbinarymultilangage gLevelOutput;
int                     gLevelOutputSize;
...
#ifndef SPECCY
#define REPLACEMENTCHARS (109+16+9)
...
string ConvertString(string inputstring)
{
	...
	*/
...
#ifdef CUTWORDALLOWED // TODO Amiga et language==19
	if (gError)
	{
	...
	if ( gError )
	{
#ifdef CUTWORDALLOWED // TODO Amiga et langage==19
...
// ------------------------------------------------------
// Write seven texts, for time up and stats
// For all langages
void SaveTimeUp()
{
	if (gStringTimeUpDeaths[0].size() != 7)
	{
		printf("ERROR, the time up and stat string should make a total of 7, not %d\n", gStringTimeUpDeaths[0].size());
		return;
	}
	
	// In stringsize save size of strings.
//	int stringsize[NUMBERLANGUAGE][7];
//	for (int j = 0 ; j < NUMBERLANGUAGE; j++ )
//	{
//		for (int i = 0; i < 7; i++)
//			stringsize[j][i] = gStringTimeUpDeaths[j][i].size();
//	}

	unsigned char buffer[168*7*NUMBERLANGUAGE+7*NUMBERLANGUAGE*2+1024];
	// Write 7 pointers * langages , then all datas. Zero terminated
//	unsigned short* ptr = (unsigned short*) buffer;
//	ptr[0] = 0;
//	for (int j = 0 ; j < NUMBERLANGUAGE; j++ )
//	  for (int i = 0; i < 7; i++)
//		if ((j*7+i)!=0)
//		  ptr[j*7+i] = ptr[(j*7+i) - 1] + stringsize[j][i - 1] + 1;

	unsigned short* ptr = (unsigned short*) buffer;
	unsigned char*  ptrdata = buffer + (7 * 2) * NUMBERLANGUAGE;
	unsigned short  datapos;

	datapos = (7 * 2) * NUMBERLANGUAGE; // Data start ofter pointer table
	...
	for (int j = 0 ; j < NUMBERLANGUAGE; j++ )
	{
		for (int i = 0; i < 7; i++)
		{
			memcpy(ptrdata, gStringTimeUpDeaths[j][i].c_str(), gStringTimeUpDeaths[j][i].size());
			ptrdata[gStringTimeUpDeaths[j][i].size()] = 0;
			ptrdata += (gStringTimeUpDeaths[j][i].size() + 1);
			// Write offset. amiga is big indian.
			buffer[2 * (i + j * NUMBERLANGUAGE) + 0] = (datapos >> 8);
			buffer[2 * (i + j * NUMBERLANGUAGE) + 1] = (datapos & 0x00FF);

			datapos += (gStringTimeUpDeaths[j][i].size() + 1);
		}
	}

	int filesize = ptrdata - buffer;
...
int findnumberofvariations(char* levelnumber)
{
	int result = -1;
	bool ok;
	char levelname[128];
	...
	{
		result++; // Start at 0
		sprintf(levelname, "PICAMIGA\\Pic%s_alt%d.iff", levelnumber, result+1); // Start at _alt1
...
void InjectTexts(char* levelnumber)
{
	...
	// We need u8 chars, that respect the format. 32 34 38 34 32 chars. (5 lines)
	gGlobalLevelLine = gGlobalLevelStartNumber +1 ; // Skip the "level" line
	int globallinestart = gGlobalLevelLine;

	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		gStringIntro[j] = ConvertString(gStringIntro[j]);
		gGlobalLevelLine++;
		for (int i = 0 ; i<gStringTexts[j].size(); i++)
		{
			gStringTexts[j][i] = ConvertString(gStringTexts[j][i]);
			gGlobalLevelLine++;
		}
		for (int i = 0 ; i<gStringDeaths[j].size(); i++)
		{
			gStringDeaths[j][i] = ConvertString(gStringDeaths[j][i]);
			gGlobalLevelLine++;
		}
		// Reset level line
		gGlobalLevelLine = globallinestart;
	}
	...
	// -- Copy data to output.
	gLevelOutputSize = 0; // Without 140*langage pointer, just data size first
	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		// The 4 first pointers are the same, for all langage (game data)
		for (int i = 0; i < 4; i++)
			gLevelOutput.pointers[j][i] = gLevelInput.pointers[i];
	}
	...
	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		gLevelOutput.pointers[j][4] = gLevelOutputSize; // Intro text
		memcpy(&(gLevelOutput.data[gLevelOutputSize]), gStringIntro[j].c_str(), gStringIntro[j].size());
		gLevelOutputSize += gStringIntro[0].size();
	}
	// -- All texts strings
	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		for (int i = 0; i < 32; i++)
		{
			gLevelOutput.pointers[j][5 + i] = 0;
			if (gLevelInput.pointers[5+i] != 0 )
			{
				gLevelOutput.pointers[j][5+i] = gLevelOutputSize; // text
				memcpy(&(gLevelOutput.data[gLevelOutputSize]), gStringTexts[j][i].c_str(), gStringTexts[j][i].size());
				gLevelOutputSize += gStringTexts[j][i].size();
			}
			else
			{
				// Check that we do not have left string.
				if (gStringTexts[j].size() > i)
					printf("This text string is not in original level ! (%d th string) \n", i);
			}
		}
	}
	...
	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		for (int i = 0; i < 32; i++)
		{
			gLevelOutput.pointers[j][5 + 32 + i] = 0;

			if (gLevelInput.pointers[5+32 + i] != 0 )
			{
				if (i >= gStringDeaths[j].size() ) // Dummy death. Input have "none" but we do not have it in translation.
				{
					gLevelOutput.pointers[j][5+32+i] = gLevelOutputSize; // gStringDeaths text
					memcpy(&(gLevelOutput.data[gLevelOutputSize]), "NONE", 4);
					gLevelOutputSize += 4;
				}
				else
				{
					gLevelOutput.pointers[j][5+32+i] = gLevelOutputSize; // gStringDeaths text
					memcpy(&(gLevelOutput.data[gLevelOutputSize]), gStringDeaths[j][i].c_str(), gStringDeaths[j][i].size());
					gLevelOutputSize += gStringDeaths[j][i].size();
				}
			}
			else
			{
				// Check that we do not have left string.
				if (gStringDeaths[j].size() > i)
					printf("This death string is not in original level ! (%d th string) \n", i);
			}
		}
	}
	// Final pointer is art data
	int nbvariations = findnumberofvariations(levelnumber); // 0 if no variations.

	unsigned char artdata[10240]; // Iff file is 9332 bytes exactly
	char artlevelname[128];
    sprintf(artlevelname, "PICAMIGA\\Pic%s.iff", levelnumber);
	FILE* fileart;
	fileart = fopen(artlevelname, "rb");
	if (!fileart)
	{
		printf("ERROR Art is missing ! (%s) \n", artlevelname);
		return;
	}
	fread(artdata, 1, 10240, fileart); // Read art data
	fclose(fileart);

	// Read data from buffer.
	// Header is $74
	// Palette is at $3c. 3 bytes per color
	unsigned short palette[16];
	for (int i = 0 ; i < 16 ; i++)
	{
		unsigned char r, g, b;
		r = artdata[0x3c + i * 3 + 0];
		g = artdata[0x3c + i * 3 + 1];
		b = artdata[0x3c + i * 3 + 2];
		// printf("%d %d %d\n", r, g, b);
		palette[i] = ((r >> 4)<<8) | ((g >> 4)<<4) | (b >> 4);
	}
	// Data start after $74
	// A plane is 96*128 = 12*128 = 1536. on 6 planes
	// We want planes side by side
	for (int j=0; j< NUMBERLANGUAGE; j++)
		gLevelOutput.pointers[j][5 + 32 + 32] = gLevelOutputSize;
	// 3 zero and then number of variations
	gLevelOutput.data[gLevelOutputSize] = 0;
	gLevelOutput.data[gLevelOutputSize+1] = 0;
	gLevelOutput.data[gLevelOutputSize+2] = 0;
	gLevelOutput.data[gLevelOutputSize+3] = nbvariations;
	gLevelOutputSize += 4;
	// Write palette
	for (int i = 0 ; i < 16 ; i++)
	{
		gLevelOutput.data[gLevelOutputSize+0] = (palette[i] >> 8);
		gLevelOutput.data[gLevelOutputSize+1] = (palette[i] & 0x00FF );
		gLevelOutputSize += 2;
	}


#if 0
	// -- Here I supposed IFF was plane after planes.
	// Seem it is already side by side planes, so only need to copy data
	// Now write data of art. Line side by side for each of the 6 planes.
	// Line is 12 bytes. 128 lines on 6 planes = 9216 bytes
	unsigned char* planes[6]; // All pointers to planes bytes
	planes[0] = artdata + 0x74; // Skip header

	planes[1] = planes[0] + LINESIZE * 128;
	planes[2] = planes[1] + LINESIZE * 128;
	planes[3] = planes[2] + LINESIZE * 128;
	planes[4] = planes[3] + LINESIZE * 128;
	planes[5] = planes[4] + LINESIZE * 128;

	for (int lines = 0; lines < 128; lines++)
	{
		for (int x = 0; x < LINESIZE; x++)
		{
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*0] = *(planes[0]+x+(LINESIZE*lines));
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*1] = *(planes[1]+x+(LINESIZE*lines));
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*2] = *(planes[2]+x+(LINESIZE*lines));
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*3] = *(planes[3]+x+(LINESIZE*lines));
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*4] = *(planes[4]+x+(LINESIZE*lines));
			gLevelOutput.data[gLevelOutputSize + x + LINESIZE*5] = *(planes[5]+x+(LINESIZE*lines));
		}
		gLevelOutputSize += (LINESIZE * 6);
	}
#endif
	unsigned char* planes;
	planes = artdata + 0x74; // Skip header
#define LINESIZE 12
	for (int x = 0; x < LINESIZE*6*128; x++)
	{
		gLevelOutput.data[gLevelOutputSize+x] = planes[x];
	}
	gLevelOutputSize += LINESIZE * 6 * 128;

	// Update offset, to have them from beginning of file.
	for (int j=0; j< NUMBERLANGUAGE; j++)
	{
		for (int i = 0; i < NBPOINTER; i++)
		{
			if (gLevelOutput.pointers[j][i] != 0 || i==0)
				gLevelOutput.pointers[j][i] += (NBPOINTER * 2 * NUMBERLANGUAGE);
		}
	}

	// Convert endianness (Amiga is big endian)
	// 70 pointers for 7 langages
	unsigned char* pfile = (unsigned char*) &gLevelOutput;
	for (int i = 0; i < (NBPOINTER * 2 * NUMBERLANGUAGE); i += 2)
	{
		unsigned char temp;
		temp = pfile[i + 1];
		pfile[i + 1] = pfile[i];
	...
	if (file)
	{
		fwrite(&gLevelOutput, 1, ((6+32+32)*2*NUMBERLANGUAGE)+gLevelOutputSize, file);
		fclose(file);
	}
...
// -------------------------------------------------------------------
// Check if this column in text is supported and need to be exported
...
int main(int argc, char** argv)
{
	...
	while(!dataFile.eof()) 
	{
		...
		ignoreline = false;
		int column = 1;
		while (iline.getline((char*)buf, 8000, 0x09) && !ignoreline) // Tab delimiter
		{
			string stringcolomn(buf); // Full line UTF-8
			//cout << stringcolomn << endl; // If console well setup, it can display UTF-8
			...
			if (column==1)
			{
				...
				if (stringcolomn.rfind("Level", 0) == 0)
				{
					...
					for (int i = 0 ; i<NUMBERLANGUAGE; i++)
					{
						gStringTexts[i].clear();
						gStringDeaths[i].clear();
					}
				...
				else if (stringcolomn == "alt-amiga")
				{
					// Remove last text as we are going to replace it
					if (IsTexts)
					{
						if (gStringTexts[0].size() == 0) // was intro
						{
							IsIntro = true;
							IsTexts = false; 
						}
						...
						{
							for (int j=0; j < NUMBERLANGUAGE; j++)
								gStringTexts[j].pop_back();
						}
					}
					else if (IsDeaths)
					{
						for (int j=0; j < NUMBERLANGUAGE; j++)
							gStringDeaths[j].pop_back();
					}
					else if (IsTimeUp)
					{
						for (int j=0; j < NUMBERLANGUAGE; j++)
						    gStringTimeUpDeaths[j].pop_back();
					}
				}
				else if (stringcolomn.rfind("alt-", 0) == 0)
					ignoreline = true; // We do nothing
			...
			// Store string (or do nothing for alternative texts)
			// Check if this index is to export
			int islanguagetoexport = checkcolumn(column);
			if (islanguagetoexport>=0)
			{
				if (ignoreline)
				{
					ignoreline = false; // Do nothing, this is alt text, reset flag
				...
				{
					gStringIntro[islanguagetoexport] = stringcolomn;
					// If line was intro then next are texts
					//IsIntro = false;
					//IsTexts = true; 
				}
				else if (IsTexts)
					gStringTexts[islanguagetoexport].push_back(stringcolomn);
				else if (IsDeaths)
					gStringDeaths[islanguagetoexport].push_back(stringcolomn);
				else if (IsTimeUp)
				{
					string tempstring = ConvertString(stringcolomn);
					gStringTimeUpDeaths[islanguagetoexport].push_back(tempstring); // Store all time up string + stat string. Should be 6 + 1
				}
			}
			column++; // Go to next column
		}
